...
export default function ImageGallery() {
  ...
  const {
    ...
    queryKey: ["images"],
    queryFn: () => imageGetList(session?.sessionToken),
    enabled: !!session?.sessionToken,
  })

  const uploadMutation = useMutation({
    mutationFn: () => uploadImage(session?.sessionToken!, selectedFile!),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (imageId: number) => deleteImage(session?.sessionToken!, imageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["images"] })
      toast({
        title: "Success",
  ...
  const activateMutation = useMutation({
    mutationFn: (imageId: number) => activateImage(session?.sessionToken!, imageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["images"] })
      toast({
        title: "Success",
  ...
  const handleActivate = (imageId: number) => {
    activateMutation.mutate(imageId)
  }
      ...
      <Card className="p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button onClick={handleUpload} disabled={!selectedFile || uploadMutation.isPending}>
            <Upload className="mr-2 h-4 w-4" /> {uploadMutation.isPending ? "Uploading..." : i("upload")}
          </Button>
        </div>
      </Card>
            ...
            : images.map((image) => (
              <Card key={image.id} className="flex flex-col overflow-hidden">
                ...
                <CardFooter className="flex justify-between p-4 bg-gray-50">
                  <Button
                    onClick={() => handleActivate(image.id)}
                    disabled={image.isActive !== 0 || activateMutation.isPending}
                    variant={image.isActive !== 0 ? "outline" : "default"}
                    size="sm"
                  >
                    <CheckCircle className="mr-2 h-4 w-4" />
                  ...
                  <Button
                    onClick={() => handleDelete(image.id)}
                    disabled={image.isActive !== 0 || deleteMutation.isPending}
                    variant="destructive"
                    size="sm"
                  >
                    <Trash2 className="mr-2 h-4 w-4" /> {i("delete")}
